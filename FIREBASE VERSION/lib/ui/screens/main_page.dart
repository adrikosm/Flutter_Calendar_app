  ...
  Widget fullCalendarView() {
    ...
    return Container(
      ...
      child: TableCalendar(
        ...
        onDaySelected: (DateTime date, DateTime focus) {
          setState(() {
            selectedDate = date;
          });
        },
...
// Format date
  ...
  editTaskView(BuildContext context, TaskModel task, String taskID) {
      ...
      builder: (context, setState) {
        return Container(
          ...
          child: Align(
            ...
            child: bottomSheet(task, taskID, setState),
          ),
        );
      },
    ));
  ...
  bottomSheet(TaskModel task, String taskID, StateSetter setState) {
    return SingleChildScrollView(
      child: Column(
        children: [
          ...
          taskCompletedButton(task, taskID),
          const SizedBox(
            height: 15,
          ),
          bottomRowButtons(task, taskID),
        ],
      ),
    );
  }

  bottomRowButtons(TaskModel task, String taskID) {
    return Container(
      margin: const EdgeInsets.only(top: 10),
      child: Row(children: <Widget>[
        cancelButton(),
        const Spacer(),
        updateButton(task, taskID),
      ]),
    );
  }

  updateButton(TaskModel task, String taskID) {
    return Container(
        ...
        child: ElevatedButton(
          ...
          onPressed: () {
            setState(() {
              ...
              TaskController.updateTask(task, taskID);
              Get.back();
            });
          },
          child: const Text(
  ...
  deleteButton(TaskModel task, String taskID) {
    return Container(
  ...
  taskCompletedButton(TaskModel task, String taskID) {
  ...
  _getTimeFromUser(bool isStartTime, StateSetter setState) async {
    ...
    if (pickedTime == null) {
      ...
      setState(() {
        defaultStartTime = formatedTime;
      });
    } else {
      setState(() {
        defaultEndTime = formatedTime;
      });
    }
  }
